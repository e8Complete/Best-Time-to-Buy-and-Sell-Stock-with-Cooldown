...
var maxProfit = function(prices) {
  ...
  if(lastDay === 2){ //We have to buy and sel the next day, or not buy at all
    ...
    stocks[day0][have] = -prices[day0]; //The first day (day0): if we buy the stock, the our balance is negative whatever the stock price is that day
    ...
    stocks[day1][have] = Math.max(stocks[day0][have], stocks[day0][dontHave] - prices[day1]); //The second day (day1): if we have the stock, it means we either bought it yesterday and didn't sell today, or we didn't have it yesterday, and we bought it today to todays price.
    ...
    for(let thisDay = 2; thisDay < lastDay; thisDay++){ //Now we fill out the rest of our array.
      ...
      stocks[thisDay][have] = Math.max(stocks[thisDay-1][have], stocks[thisDay-2][dontHave] - prices[thisDay]);
    }
